...
def rows(input, separator):
    lines = input.split("\n")
    rows = []
    for line in lines:
        chars = line.strip().split(separator)
        cols = _to_ints(chars)
        rows.append(cols)
    return rows

def checksum_max_min_differences(sheet, separator):
    total = 0
    for row in rows(sheet, separator):
        total = total + (max(row) - min(row))
    return total

def checksum_evenly_divisible(sheet, separator):
    total = 0
    for row in rows(sheet, separator):
        total = total + even_dividend(row)
    return total

def even_dividend(ints):
    for a, b in permutations(ints, 2):
        dividend, remainder = divmod(a, b)
        if remainder == 0:
            return dividend

    raise ValueError("No evenly divisible pairs found in {}".format(ints))

def test_checksum_spreadsheet_max_min():
    sheet = """5 1 9 5
    7 5 3
    2 4 6 8"""
    assert checksum_max_min_differences(sheet, " ") == 18

def test_checksum_spreadsheet_evenly_divisible():
    sheet = """5 9 2 8
    9 4 7 3
    3 8 6 5"""
    assert checksum_evenly_divisible(sheet, " ") == 9
...
print("Part one:")
print(checksum_max_min_differences(input, "\t"))

print("Part two:")
print(checksum_evenly_divisible(input, "\t"))